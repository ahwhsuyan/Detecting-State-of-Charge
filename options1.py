...
def get_options(args=None):
    ...
    parser.add_argument(
        "--test_dataset",
        type=str,
        default="dnn_datasets/detection_test_syn_comp_model_3.pt",
    ...
    parser.add_argument(
        "--dataset_size", type=int, default=8864, help="Dataset size for training",
    )
    ...
    parser.add_argument(
        "--analyze_complexity",
        action="store_true",
        help="分析模型复杂性（参数量、FLOPs、推理时间）",
    )

    parser.add_argument(
        "--complexity_only",
        action="store_true",
        help="仅进行模型复杂性分析，不进行训练",
    )
